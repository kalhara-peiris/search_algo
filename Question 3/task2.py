...
def uniform_cost_search(maze, start_row, start_col, goal_row, goal_col):
    # Priority queue for UCS, initialized with start node
    pq = [(0, (start_row, start_col))]  # (cost, (row, col))
        ...
        for neighbor in get_neighbors(row, col):
            ...
            if maze[nr][nc] != 'B' and (nr, nc) not in visited:  # Skip barriers and visited nodes
                ...
                if (nr, nc) not in g_score or tentative_g_score < g_score[(nr, nc)]:
                    ...
                    heapq.heappush(pq, (tentative_g_score, (nr, nc)))
        
    return visited_nodes, total_time, []  # In case no path is found

# If this file is run directly, test UCS on a sample maze
if __name__ == "__main__":
    ...
    visited_nodes, time_taken, path = uniform_cost_search(maze, start_row, start_col, goal_row, goal_col)
